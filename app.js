...
function closeSideOfAdminLogin() {
    document.getElementById("adminLogin").className = 'adminPanel';
}
function e() {
...
function FoodItems(category, name, price, colors, stock, sizes) {
...
var FoodItems = {
    Bakery: {
        chocklate: new Fooditems("Bakery", " chocklate", 900, "chocklate", "Yes"),
        cupcake: new Fooditems("Bakery", "cupcake", 500, "Brown", "Yes"),
        Donuts: new Fooditems("Bakery", "Donuts", 300, "salmon", "Yes", ),
        Genoisecake: new Fooditems("Bakery", "Genoisecake", 1400, "whitesmoke", "No"),
        Redvelvet: new Fooditems("Bakery", "Redvelvet", 1000, "red", "Yes"),
        Operacake: new Fooditems("Bakery", "Operacake", 2500, "purple", "No"),
    },
    Veggies: {
        Pesto: new Fooditems("Veggies", "Pesto", 500, "Grey/Light-Red", "Yes"),
        Swapableveggies: new Fooditems("Veggies", "Swapable Veggies", 300, "Blue/Off-White", "No"),
        Veganbowl: new Fooditems("Veggies", "Vegan Bowl", 450, "Green-Dark", "Yes"),
        Mexicanbowl: new Fooditems("Veggies", "Mexican Bowl", 500, "Red/Mehrun", "No"),
        Shrrombowl: new Fooditems("Veggies", "Shroom Bowl", 250, "Black/Blue/Grey", "Yes"),
        Microgreens: new Fooditems("Veggies", "Microgreens", 200, "Blue/Dark-Blue", "Yes"),
    },
    Spicy: {
        Burgerfries: new Fooditems("Spicy", "Burger&Fries", 500, "Mehrun", "Yes"),
        Pizza: new Fooditems("Spicy", "Pizza", 300, "Light-Yello", "No"),
        Hotdog: new Fooditems("Spicy", "Hotdog", 150, "Grey", "Yes"),
        Hamburger: new Fooditems("Spicy", "Hamburger", 200, "Light-Grey", "Yes"),
        Friedchicken: new Fooditems("Spicy", "Fried Chicken", 400, "Black", "Yes"),
        Nuggets: new Fooditems("Spicy", "Nuggets", 250, "Black/Grey", "No")
    },
}
function filter() {
...
function details(productName, srcc) {
    ...
    for (var key in fooditem) {
        for (var key2 in fooditem[key]) {
            if (name === key2) {
                swal({
                    title: "Name : " + fooditem[key][key2].name,
                    text: "Category : " + "( " + fooditem[key][key2].category + " )  "
                        + " --- Color : ( " + fooditem[key][key2].color + " ) "
                        + " --- Stock  : ( " + fooditem[key][key2].stock + " ) "
                        
                        + " --- Price : ( RS." + fooditem[key][key2].price + " ) ",
                    textColor: "red",
                    imageUrl: src,
                    imageWidth: 300,
                    imageHeight: 250,
            ...
            else if (name === fooditem[key][key2].name) {
                swal({
                    title: "Name : " + fooditem[key][key2].name,
                    text: "Category : " + "( " + fooditem[key][key2].category + " )  "
                        + " --- Color : ( " + fooditem[key][key2].color + " ) "
...
function sellProductInfoGet(id) {
    ...
    for (var key in fooditem) {
        for (var key2 in fooditem[key]) {
            if (id === key2) {
                document.getElementById('productName').innerText = fooditem[key][key2].name;
                document.getElementById("date").innerText = todayDate + "/" + todayMonth + "/" + todayyear;
                document.getElementById('priceOfProduct').innerText = fooditem[key][key2].price;
    ...
    for (var i = 0; i < quantity.length; i++) {
        ...
        if (flag != true) {
            ...
            for (var key in fooditem) {
                for (var key2 in fooditem[key]) {
                    if (nameProduct === key2) {
                        var NAME = fooditem[key][key2].name;
                        var PRICE = fooditem[key][key2].price;
                        var QUANTITY = Number(quantity);
                        var TOTALPRICE = QUANTITY * fooditem[key][key2].price;
                        var date = new Date();
                        var todayDate = date.getDate();
                        var todayMonth = date.getMonth() + 1;
...
function insertData(id) {
    ...
    if (id === "Bakery") {
        ...
        counterForFood++;
        var maintarget = document.getElementById("newAddedOfBakery");
        ...
        subDiv.setAttribute("id", 'f' + counterForFood);
        subDiv.setAttribute("onmouseover", 'changee(' + "'" + 'f' + counterForFood + "'" + ')');
        subDiv.setAttribute("onmouseout", 'changeeag(' + "'" + 'f' + counterForFood + "'" + ')');


        var img = document.createElement("img");
        img.setAttribute("src", "b7.jpg");
        ...
        plus.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './b7.jpg' + "'" + ')');
        ...
        p.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './b7.jpg' + "'" + ')')
        ...
        editBtn.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './b7.jpg' + "'" + ')')
        ...
        var a = document.getElementById("fooditemAdd");
    ...
    if (id === "veggies") {
        ...
        for (var key in fooditem) {
            if (key === id) {
                fooditem[key].newProduct = new fooditem(id, name, price, color, stock)
            }
        }
        counterForFood++;
        var maintarget = document.getElementById("newAddedOfVeggies");
        ...
        var subDiv = document.createElement("div");
        subDiv.setAttribute("class", "imgCover");
        subDiv.setAttribute("id", 'c' + counterForFood);
        subDiv.setAttribute("onmouseover", 'changee(' + "'" + 'c' + counterForFood + "'" + ')');
        subDiv.setAttribute("onmouseout", 'changeeag(' + "'" + 'c' + counterForFood + "'" + ')');

        var img = document.createElement("img");
        img.setAttribute("src", "v7.jpg");
        img.setAttribute("width", "200px");
        img.setAttribute("height", "150px");
        img.setAttribute("alt", "Please wait Loading...")
        subDiv.appendChild(img);
        ...
        var plus = document.createElement("div");
        plus.setAttribute("class", "b");
        plus.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './v7.jpg' + "'" + ')');
        var plustext = document.createTextNode("+");
        plus.appendChild(plustext);
        subDiv.appendChild(plus);
        mainDiv.appendChild(subDiv);
        ...
        p.setAttribute("class", "proInfoN");
        p.setAttribute("id", objName + "1");
        p.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './v7.jpg' + "'" + ')')
        var ptext = document.createTextNode(name);
        p.appendChild(ptext);
        ...
        editBtn.setAttribute("class", "adminBtnsEdit");
        editBtn.setAttribute("title", "More");
        editBtn.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './v7.jpg' + "'" + ')')
        var editBtnFa = document.createElement("i");
        editBtnFa.setAttribute("class", "fa fa-arrow-up");
        editBtn.appendChild(editBtnFa);
        var editBtnText = document.createTextNode(" MORE");
    ...
    if (id === "Spicy") {
        ...
        for (var key in fooditem) {
            if (key === id) {
                fooditem[key].newProduct = new Products(id, name, price, color, stock)
            }
        }
        counterForFood++;
        var maintarget = document.getElementById("newAddedOfSpicy");
        ...
        var subDiv = document.createElement("div");
        subDiv.setAttribute("class", "imgCover");
        subDiv.setAttribute("id", 'w' + counterForFood);
        subDiv.setAttribute("onmouseover", 'changee(' + "'" + 'w' + counterForFood + "'" + ')');
        subDiv.setAttribute("onmouseout", 'changeeag(' + "'" + 'w' + counterForFood + "'" + ')');

        var img = document.createElement("img");
        img.setAttribute("src", "s7.jpg");
        img.setAttribute("width", "200px");
        img.setAttribute("height", "150px");
        img.setAttribute("alt", "Please wait Loading...")
        ...
        var plus = document.createElement("div");
        plus.setAttribute("class", "b");
        plus.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './s7.jpg' + "'" + ')');
        var plustext = document.createTextNode("+");
        plus.appendChild(plustext);
        subDiv.appendChild(plus);
        mainDiv.appendChild(subDiv);
        ...
        p.setAttribute("class", "proInfoN");
        p.setAttribute("id", objName + "1");
        p.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './s7.jpg' + "'" + ')')
        var ptext = document.createTextNode(name);
        p.appendChild(ptext);
        ...
        editBtn.setAttribute("class", "adminBtnsEdit");
        editBtn.setAttribute("title", "More");
        editBtn.setAttribute("onclick", 'details(' + "'" + name + "'" + ',' + "'" + './s7.jpg' + "'" + ')')
        var editBtnFa = document.createElement("i");
        editBtnFa.setAttribute("class", "fa fa-arrow-up");
        editBtn.appendChild(editBtnFa);
        var editBtnText = document.createTextNode(" MORE");
        ...
        maintarget.appendChild(mainDiv);
        var a = document.getElementById("SpicyAdd");
        a.removeAttribute("onclick");
        a.style.backgroundColor = "rgb(164, 252, 164)"
    }
}
